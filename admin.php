...
if (in_array($text, $textadmin) || $datain == "admin") {
    ...
    if ($userdata['type'] == "hiddify") {
        ...
        savedata("save", "password", "null");
        return;
    } elseif ($userdata['type'] == "s_ui" || $userdata['type'] == "WGDashboard" || $userdata['type'] == "x-ui_single" || $userdata['type'] == "candy_agent") {
...
} elseif ($datain == "tetra_offline" && $adminrulecheck['rule'] == "administrator") {
    // نمایش کیبورد تنظیمات Tetra
    sendmessage($from_id, $textbotlang['users']['selectoption'], $tetrakeyboard, 'HTML');

} elseif ($text == $textbotlang['keyboard']['apitetra'] && $adminrulecheck['rule'] == "administrator") {
    // گرفتن مقدار فعلی از دیتابیس
...
if ($datain == "settimecornremove" && $adminrulecheck['rule'] == "administrator") {
...
} elseif ($datain == "tetrasetting" && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $tetrakeyboard, 'HTML');
    ...
    $tetra = getPaySettingValue('tetrastatus', 'offtetra');
    ...
    $cartotcartstatus = [
     ...
     $tetra_status = [
        'ontetra' => $textbotlang['Admin']['Status']['statuson'],
        'offtetra' => $textbotlang['Admin']['Status']['statusoff']
    ][$tetra];
    $plisiostatus = [
    ...
    $Bot_Status = json_encode([
        'inline_keyboard' => [
            ...
            [
                ...
                ['text' => $textbotlang['keyboard']['cartToCartGateway'], 'callback_data' => "cartsetting"],
            ],
            [
                ['text' => $textbotlang['keyboard']['settings'], 'callback_data' => "tetra_offline"],
                ['text' => $tetra_status, 'callback_data' => "editpayment-tetrastatus-$tetra"],
    ...
    } elseif ($type == "tetrastatus") {
        if ($value == "ontetra") {
            $valuenew = "offtetra";
        } else {
            $valuenew = "ontetra";
        }
        update("PaySetting", "ValuePay", $valuenew, "NamePay", "tetrastatus");
    }
...
} elseif ($text == $textbotlang['keyboard']['cashbacktetra']) {
    sendmessage($from_id, $textbotlang['Admin']['SettingPayment']['sendValue'], $backadmin, 'HTML');
    step("getcashtetra", $from_id);
} elseif ($user['step'] == "getcashtetra") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['Admin']['agent']['invalidValue'], $backadmin, 'HTML');
        return;
    }
...
} elseif ($user['step'] == "minbalancetetra") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['Admin']['agent']['invalidValue'], $backadmin, 'HTML');
        return;
    }
...
} elseif ($user['step'] == "maxbalancetetra") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['Admin']['agent']['invalidValue'], $backadmin, 'HTML');
        return;
    }
    ...
    if ($text) {
        ...
        update("PaySetting", "ValuePay", $data, "NamePay", "helpcart");
// --- بخش شروع تنظیم آموزش (وقتی دکمه زده می‌شود) ---
} elseif ($text == $textbotlang['keyboard']['setEducationtetra'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, "لطفا متن، عکس یا ویدیو آموزشی درگاه Tetra98 را ارسال کنید:", $backadmin, 'HTML');
    step("gethelptetra", $from_id);

// --- بخش ذخیره‌سازی آموزش (کد شما) ---
} elseif ($user['step'] == "gethelptetra") {
    $success_msg = "✅ آموزش درگاه Tetra98 با موفقیت ذخیره شد.";
    ...
    // بازگشت به خانه و نمایش پنل
    sendmessage($from_id, $success_msg, $setting_panel, 'HTML');
    step("home", $from_id);
    } else {
